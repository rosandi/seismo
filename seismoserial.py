...
allowed_cmd=['msr', 'trig', 'dt', 'avg', 'ping', 'chn', 'vref', 'pick', 'info']
...
def deviceCommand(scmd):
    ...
    if cmdval[0] == 'msr' or cmdval[0]=='trig':
        vals=[]
        savesend(scmd)
        ntry=0
        ndat=int(cmdval[1])
        ...
        waittrig=True if cmdval[0] == 'trig' else False
...
# def triggerMeasure(datalen,delay=0):
    
    # # line: "CONFIRMED: navg dt cmask buff_len version"
    # sline=deviceCommand('info 0').split()
    # cmask=int(sline[3])
    # vref=float(sline[4])

    # # 0 means pick mode, pcode='0'
    # savesend('trig 0')
    # qsera.queue.clear()
    # vals=[]
...
def directMeasure(datalen,delay=0.005):
...
#        try:
#            a=float(qsera.get())*vref/MAXADC - vref/2.0
#            vals.append(a)
#            i+=1
#        except:
#            pass
...
if __name__ == "__main__":
    ...
    for arg in sys.argv:
        ...
        if arg.find('channel=') == 0:
            chan=int(arg.replace('channel=',''))
        if arg.find('avg=') == 0:
        ...
        if arg.find('stream') == 0: # use no queue
            dostream=True
    ...
    if datalog:
        print(deviceCommand('chn '+str(chan))
        ...
        #
        # dostream means: read without the help of queue
        #
        
        if dostream:
            ev.set()
            ser.write(bytes('$pick a;','ascii'))
            sleep(1)
            serthr.join()
